TokoVoipConfig = {
	...
	radioKey = 19, -- Keybind used to talk on the radio
	keyProximity = 83, -- Keybind used to switch the proximity mode
	...
	plugin_data = {
		...
		TSChannel = "TODO",
		TSPassword = "ExtraVision123", -- TeamSpeak channel password (can be empty)
		...
		TSChannelWait = "TODO[waiting-channel]",

		-- Blocking screen informations
		TSServer = "TOKO-IP", -- TeamSpeak server address to be displayed on blocking screen
		...
		TSChannelWhitelist = { -- Black screen will not be displayed when users are in those TS channels
			"ExtraVision | Managment - Room",
			"ExtraVision | Staff - Room",
		},
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = 'Player - [' .. GetPlayerServerId(PlayerId()) .. ']'
		end);
