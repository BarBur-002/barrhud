...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent("ym-hud:server:requestTable")
end)
...
Citizen.CreateThread(function() 
    while true do
        Citizen.Wait(1000)
        if IsPedInAnyVehicle(PlayerPedId()) and YMHud then
            DisplayRadar(true)
            SendNUIMessage({
                action = "car",
                show = true,
...
Citizen.CreateThread(function()
	while true do
        ...
        if DoesEntityExist(veh) then
            ...
            TriggerEvent("ym-hud:client:UpdateDrivingMeters", true, math.floor(vehiclesKHM[plate]))
        end
	...
	while true do
        ...
        if DoesEntityExist(veh) then
            ...
            if vehiclesKHM[plate] and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                TriggerServerEvent("ym-hud:server:vehiclesKHM", plate, vehiclesKHM[plate])
            end
        end
	end
end)

RegisterNetEvent("ym-hud:client:vehiclesKHM")
AddEventHandler("ym-hud:client:vehiclesKHM", function(plate,kmh)
    vehiclesKHM[plate] = kmh
end)

RegisterNetEvent("ym-hud:client:vehiclesKHMTable")
AddEventHandler("ym-hud:client:vehiclesKHMTable", function(table)
    vehiclesKHM = table
end)
...
RegisterNetEvent('ym-hud:client:ToggleHarness')
AddEventHandler('ym-hud:client:ToggleHarness', function(toggle)
    SendNUIMessage({
        action = "harness",
        toggle = toggle
    })
...
RegisterNetEvent('ym-hud:client:UpdateDrivingMeters')
AddEventHandler('ym-hud:client:UpdateDrivingMeters', function(toggle, amount)
    SendNUIMessage({
...
RegisterNetEvent('ym-hud:client:UpdateVoiceProximity')
AddEventHandler('ym-hud:client:UpdateVoiceProximity', function(Proximity)
    SendNUIMessage({
        action = "proximity",
        prox = Proximity
    })
...
RegisterNetEvent('ym-hud:client:ProximityActive')
AddEventHandler('ym-hud:client:ProximityActive', function(active)
    SendNUIMessage({
        action = "talking",
        IsTalking = active
    })
